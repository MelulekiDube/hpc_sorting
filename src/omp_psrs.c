//https://github.com/Fitzpasd/Parallel-sort-by-regular-sampling
#include <omp.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main(int argc, char *argv[]){
	...
	psrs_sort(fc->arr, fc->size);
	double end = omp_get_wtime();
	char file_name[200];
	sprintf(file_name, "%s_%d", "omp_psrs_result", fc->size);
	FILE* file = fopen(file_name, "w");
	fprintf(file, "Duration: %0.9f\n", (end-start));
	fclose(file);
	return 0;
}